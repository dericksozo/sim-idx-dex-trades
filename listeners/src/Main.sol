...
contract Triggers is BaseTriggers {
    function triggers() external virtual override {
        Listener psmListener = new Listener();
...
contract Listener is PSM$OnBuyGemFunction, PSM$OnSellGemFunction {
    event DexTrade(DexTradeData);

    function onBuyGemFunction(
        FunctionContext memory ctx,
        PSM$BuyGemFunctionInputs memory inputs
    ) external override {
    ...
    function onSellGemFunction(
        FunctionContext memory ctx,
        PSM$SellGemFunctionInputs memory inputs
    ) external override {
