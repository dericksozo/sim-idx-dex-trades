...
import {
    ...
    IUniswapV3Pool,
    IUniswapV2Pair
} from "./interfaces/Uniswap/UniswapInterfaces.sol";
...
contract DexUtils {
    ...
    constructor() {
        ...
        pancakeSwapV3Factories[8453] = 0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865;
    ...
    function getUniswapV3Pool(address token0, address token1, address factory) internal returns (address) {
        ...
        uint128 p500Liquidity = p500 == address(0) ? 0 : IUniswapV3Pool(p500).liquidity();
        uint128 p100Liquidity = p100 == address(0) ? 0 : IUniswapV3Pool(p100).liquidity();
        uint128 p10000Liquidity = p10000 == address(0) ? 0 : IUniswapV3Pool(p10000).liquidity();
        uint128 p3000Liquidity = p3000 == address(0) ? 0 : IUniswapV3Pool(p3000).liquidity();
        if (p500Liquidity >= p100Liquidity && p500Liquidity >= p10000Liquidity && p500Liquidity >= p3000Liquidity) {
            return p500;
        }
        if (p100Liquidity >= p500Liquidity && p100Liquidity >= p10000Liquidity && p100Liquidity >= p3000Liquidity) {
            return p100;
        }
        if (p10000Liquidity >= p500Liquidity && p10000Liquidity >= p100Liquidity && p10000Liquidity >= p3000Liquidity) {
            return p10000;
        }
        return p3000;
    }
    ...
    function checkIfDodoV2PoolWithFactory(address pool, address factory) internal view returns (bool) {
        ...
        address baseToken = IDODOPool(pool)._BASE_TOKEN_();
        address quoteToken = IDODOPool(pool)._QUOTE_TOKEN_();
        address[] memory pools = IDODOPoolFactory(factory).getDODOPool(baseToken, quoteToken);
    ...
    function checkIfDodoV2Pool(address pool) internal view returns (bool) {
        ...
        return checkIfDodoV2PoolWithFactory(pool, dppFactory) || checkIfDodoV2PoolWithFactory(pool, dspFactory)
    ...
    function getCurvePoolToFromTokens(address pool, int128 bId, int128 sId) internal returns (address, address) {
        ...
        (success, returnData) = pool.call(abi.encodeWithSignature("coins(uint256)", boughtId));
        if (success) {
            tokenTo = abi.decode(returnData, (address));
            (success, returnData) = pool.call(abi.encodeWithSignature("coins(uint256)", soldId));
            if (success) {
            ...
            (success, returnData) = pool.call(abi.encodeWithSignature("coins(int128)", bId));
            if (success) {
                tokenTo = abi.decode(returnData, (address));
                (success, returnData) = pool.call(abi.encodeWithSignature("coins(int128)", sId));
                if (success) {
                    tokenFrom = abi.decode(returnData, (address));
                    return (tokenTo, tokenFrom);
                }
